...
//         const allFiles = getAllFiles();
//         const selectedNonImages = allFiles.filter(
//           (file) => selectedFiles.includes(file._id) && !file.mimetype?.startsWith("image/")
//         );
//         for (const file of selectedNonImages) {
//           await onDownloadFile(file._id, file.displayName);
//         }
//       }
...
//                                         autoFocus
//                                         onBlur={() => {
//                                           handleFileNameEdit(file._id, editFileName);
...
//                                   ) : (
//                                     <p
//                                       className="text-sm font-medium text-gray-800 truncate cursor-pointer hover:text-indigo-600 transition-colors file-name"
//                                       onClick={(e) => {
//                                         e.stopPropagation();
//                                         if (canManageFiles) {
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                            {sessionFiles.map((file) => (
                              <motion.div
                                ...
                                onClick={(e) => {
                                  // Only handle card click if not clicking on name or action buttons
                                  if (
                                    !e.target.closest('.file-name') &&
                                    !e.target.closest('.file-actions')
                                  ) {
